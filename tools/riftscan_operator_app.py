...
POST_UPDATE_BASELINE_CMD = REPO_ROOT / "scripts" / "run-riftscan-post-update-baseline.cmd"
POST_UPDATE_BASELINE_DIR = REPO_ROOT / "handoffs" / "current" / "post-update-baseline"
...
MOVEMENT_EXECUTION_GATE_CMD = REPO_ROOT / "scripts" / "run-riftscan-movement-execution-gate.cmd"
MOVEMENT_EXECUTION_GATE_DIR = REPO_ROOT / "handoffs" / "current" / "movement-execution-gate"
...
ALLOWLIST = [
    ...
    "scripts/live-test-riftscan.cmd",
...
def latest_post_update_baseline_summary() -> dict[str, Any]:
    if not POST_UPDATE_BASELINE_SUMMARY.exists():
        return {
            "status": "none",
            "reason": "post-update baseline summary has not been generated",
            "expected_report_path": rel(POST_UPDATE_BASELINE_REPORT),
            "expected_summary_path": rel(POST_UPDATE_BASELINE_SUMMARY),
            "expected_log_path": rel(POST_UPDATE_BASELINE_LOG),
        }

    summary = load_json(POST_UPDATE_BASELINE_SUMMARY)
    if summary.get("_missing") or summary.get("_error"):
        return {
            "status": "error",
            "summary_path": rel(POST_UPDATE_BASELINE_SUMMARY),
            "summary": summary,
        }

    return {
        "status": "present",
        "report_path": rel(POST_UPDATE_BASELINE_REPORT),
        "summary_path": rel(POST_UPDATE_BASELINE_SUMMARY),
        "log_path": rel(POST_UPDATE_BASELINE_LOG),
        "summary": summary,
...
def latest_capture_plan_check_summary() -> dict[str, Any]:
    if not CAPTURE_PLAN_CHECK_SUMMARY.exists():
        return {
            "status": "none",
            "reason": "capture plan check summary has not been generated",
            "expected_report_path": rel(CAPTURE_PLAN_CHECK_REPORT),
            "expected_summary_path": rel(CAPTURE_PLAN_CHECK_SUMMARY),
            "expected_log_path": rel(CAPTURE_PLAN_CHECK_LOG),
        }

    summary = load_json(CAPTURE_PLAN_CHECK_SUMMARY)
    if summary.get("_missing") or summary.get("_error"):
        return {
            "status": "error",
            "summary_path": rel(CAPTURE_PLAN_CHECK_SUMMARY),
            "summary": summary,
        }

    return {
        "status": "present",
        "report_path": rel(CAPTURE_PLAN_CHECK_REPORT),
        "summary_path": rel(CAPTURE_PLAN_CHECK_SUMMARY),
        "log_path": rel(CAPTURE_PLAN_CHECK_LOG),
        "summary": summary,
        "report_exists": CAPTURE_PLAN_CHECK_REPORT.exists(),
        "log_exists": CAPTURE_PLAN_CHECK_LOG.exists(),
    }
...
def latest_movement_execution_gate_summary() -> dict[str, Any]:
    if not MOVEMENT_EXECUTION_GATE_SUMMARY.exists():
        return {
            "status": "none",
            "reason": "movement execution gate summary has not been generated",
            "expected_report_path": rel(MOVEMENT_EXECUTION_GATE_REPORT),
            "expected_summary_path": rel(MOVEMENT_EXECUTION_GATE_SUMMARY),
            "expected_log_path": rel(MOVEMENT_EXECUTION_GATE_LOG),
        }

    summary = load_json(MOVEMENT_EXECUTION_GATE_SUMMARY)
    if summary.get("_missing") or summary.get("_error"):
        return {
            "status": "error",
            "summary_path": rel(MOVEMENT_EXECUTION_GATE_SUMMARY),
            "summary": summary,
        }

    return {
        "status": "present",
        "report_path": rel(MOVEMENT_EXECUTION_GATE_REPORT),
        "summary_path": rel(MOVEMENT_EXECUTION_GATE_SUMMARY),
        "log_path": rel(MOVEMENT_EXECUTION_GATE_LOG),
        "summary": summary,
...
def latest_capture_plan_gate_details(capture_plan: dict[str, Any] | None) -> dict[str, Any]:
    capture_plan = capture_plan if isinstance(capture_plan, dict) else {"status": "none"}
    manifest = capture_plan.get("manifest") if isinstance(capture_plan.get("manifest"), dict) else {}
    valid_metadata_only = (
        capture_plan.get("status") == "present"
        and manifest.get("schema_version") == "riftscan.focus_gated_capture_plan.v1"
        and manifest.get("status") == "capture_plan_created"
        and manifest.get("metadata_only") is True
        and manifest.get("capture_started") is False
        and manifest.get("capture_completed") is False
    )
    return {
        "artifact_status": capture_plan.get("status"),
        "status": "valid_metadata_only" if valid_metadata_only else "not_valid_metadata_only",
        "latest_plan": capture_plan.get("latest_plan"),
        "manifest_path": capture_plan.get("manifest_path"),
        "handoff_path": capture_plan.get("handoff_path"),
        "schema_version": manifest.get("schema_version"),
        "plan_status": manifest.get("status"),
...
def build_current_workflow_gate(
    *,
    ...
    movement_execution_gate: dict[str, Any] | None = None,
) -> dict[str, Any]:
    ...
    movement_execution_gate_details = latest_simple_check_gate_details(movement_execution_gate, "Movement Execution Gate")
    movement_execution_summary = (
        movement_execution_gate.get("summary")
        if isinstance(movement_execution_gate, dict) and isinstance(movement_execution_gate.get("summary"), dict)
        else {}
    )
    movement_execution_gate_details["movement_execution_allowed"] = movement_execution_summary.get("movement_execution_allowed")
    movement_execution_gate_details["expires_utc"] = movement_execution_summary.get("expires_utc")
    baseline_summary = post_update_baseline.get("summary") if isinstance(post_update_baseline.get("summary"), dict) else {}
    readiness_summary = capture_readiness.get("summary") if isinstance(capture_readiness.get("summary"), dict) else {}
    ...
    if baseline["display_status"] != "PASS":
        ...
        movement_execution_gate_details.get("display_status") == "PASS"
        and movement_execution_gate_details.get("movement_execution_allowed") is True
    ):
    ...
    elif movement_execution_gate_details.get("display_status") == "BLOCKED":
    ...
    return {
        ...
        "movement_execution_gate": movement_execution_gate_details,
...
def workflow_gate_text(gate: dict[str, Any]) -> str:
    ...
    movement_execution_gate = gate.get("movement_execution_gate") if isinstance(gate.get("movement_execution_gate"), dict) else {}
    ...
    lines = [
        ...
        f"movement_execution_gate: {movement_execution_gate.get('display_status')}",
        f"movement_execution_allowed: {movement_execution_gate.get('movement_execution_allowed')}",
        f"movement_execution_expires_utc: {movement_execution_gate.get('expires_utc')}",
...
def operator_self_test_capture_plan(
    *,
    metadata_only: bool = True,
    capture_started: bool = False,
    capture_completed: bool = False,
    status: str = "capture_plan_created",
) -> dict[str, Any]:
    return {
        "status": "present",
...
def operator_self_test_movement_execution_gate(
    display_status: str = "PASS",
    status: str = "pass",
    *,
    movement_execution_allowed: bool = True,
) -> dict[str, Any]:
    return {
        "status": "present",
        "summary": {
            "status": status,
            "display_status": display_status,
            "created_utc": "2026-05-06T00:03:00Z",
            "movement_execution_allowed": movement_execution_allowed,
            "expires_utc": "2026-05-06T00:08:00Z" if movement_execution_allowed else None,
            "blockers": [] if movement_execution_allowed else ["Focus preflight command failed before movement execution gate."],
            "paths": {
...
def run_operator_self_test() -> tuple[bool, dict[str, Any]]:
    ...
    def record(
        ...
        *,
        ...
        gate = build_current_workflow_gate(
            ...
            movement_execution_gate=movement_execution_gate,
        )
    ...
    record(
        "all gates pass without latest capture plan",
        "PASS",
        baseline=pass_baseline,
    ...
    record(
        "all gates pass with latest metadata-only capture plan",
        "PASS",
        baseline=pass_baseline,
        readiness=pass_readiness,
    ...
    record(
        ...
        expected_next_action_part="Run the final current-window Movement Execution Gate",
    )
    record(
        "all gates pass with blocked movement execution gate",
        "PASS",
        baseline=pass_baseline,
        readiness=pass_readiness,
        capture_plan=operator_self_test_capture_plan(),
        capture_plan_check=operator_self_test_simple_check(),
        movement_test_readiness=operator_self_test_simple_check(),
        movement_execution_gate=operator_self_test_movement_execution_gate(
            "BLOCKED",
            "blocked_movement_execution_not_allowed",
            movement_execution_allowed=False,
        ),
        expected_next_action_part="Resolve Movement Execution Gate blockers",
    )
    record(
        "all gates pass with movement execution gate pass",
        "PASS",
        baseline=pass_baseline,
        readiness=pass_readiness,
        capture_plan=operator_self_test_capture_plan(),
        ...
        movement_test_readiness=operator_self_test_simple_check(),
        movement_execution_gate=operator_self_test_movement_execution_gate(),
        expected_next_action_part="exact bounded move_forward command",
    )
    record(
...
class RiftScanOperatorApp(tk.Tk):
    ...
    def run_post_update_baseline(self) -> None:
        in_world = messagebox.askyesnocancel(
            "Post-Update Baseline",
            (
                "Is the CURRENT updated RIFT client fully logged in and stable in-world?\n\n"
                "Yes: run with --assume-in-world and --strict-exit-code.\n"
                "No: run the conservative baseline; BLOCKED is expected until in-world state is confirmed.\n"
                "Cancel: do nothing.\n\n"
                "This does not start capture, send input, scan memory, validate offsets, or run /reloadui."
            ),
        )
        if in_world is None:
            self.status_var.set("Post-update baseline canceled.")
            return
        ...
        def task() -> str:
            ...
            args = ["cmd", "/c", str(POST_UPDATE_BASELINE_CMD), "--strict-exit-code"]
            if in_world:
                args.append("--assume-in-world")

            exit_code, stdout, stderr = run_command(args, timeout=120)
            summary = load_json(POST_UPDATE_BASELINE_SUMMARY)
            ...
            if summary.get("_missing") or summary.get("_error"):
                ...
                blockers = [summary.get("_error") or f"Missing summary: {rel(POST_UPDATE_BASELINE_SUMMARY)}"]
                paths = {
                    "report": rel(POST_UPDATE_BASELINE_REPORT),
                    "summary": rel(POST_UPDATE_BASELINE_SUMMARY),
                    "log": rel(POST_UPDATE_BASELINE_LOG),
                }
            else:
            ...
            lines = [
                "\n=== POST-UPDATE BASELINE ===",
                f"POST-UPDATE BASELINE: {display_status}",
                f"status: {status}",
                f"Exit code: {exit_code}",
                f"In-world assertion used: {bool(in_world)}",
                "",
                "Blockers:",
            ]
            if blockers:
            ...
            lines.extend(
                [
                    "",
                    f"Report: {paths.get('report') or rel(POST_UPDATE_BASELINE_REPORT)}",
                    f"Summary: {paths.get('summary') or rel(POST_UPDATE_BASELINE_SUMMARY)}",
                    f"Log: {paths.get('log') or rel(POST_UPDATE_BASELINE_LOG)}",
                    f"Operator report: {rel(report_path)}",
                ]
            )
            ...
            lines.extend(
                [
                    "",
                    "Safety: no capture, movement/input, memory scan/read, offset validation, RiftReader validation, or /reloadui was run.",
                ]
            )
            return "\n".join(lines)
    ...
    def run_capture_plan_check(self) -> None:
        def task() -> str:
            if not CAPTURE_PLAN_CHECK_CMD.exists():
                return f"ERROR: missing {rel(CAPTURE_PLAN_CHECK_CMD)}"

            exit_code, stdout, stderr = run_command(
                ["cmd", "/c", str(CAPTURE_PLAN_CHECK_CMD), "--strict-exit-code"],
                timeout=120,
            )
            summary = load_json(CAPTURE_PLAN_CHECK_SUMMARY)
            report_path = write_operator_report()
            ...
            if summary.get("_missing") or summary.get("_error"):
                display_status = "FAIL"
                status = "summary_unavailable"
                blockers = [summary.get("_error") or f"Missing summary: {rel(CAPTURE_PLAN_CHECK_SUMMARY)}"]
                paths = {
                    "report": rel(CAPTURE_PLAN_CHECK_REPORT),
                    "summary": rel(CAPTURE_PLAN_CHECK_SUMMARY),
                    "log": rel(CAPTURE_PLAN_CHECK_LOG),
                }
            else:
                display_status = str(summary.get("display_status") or "UNKNOWN")
                status = str(summary.get("status") or "unknown")
            ...
            lines = [
                "\n=== CAPTURE PLAN CHECK ===",
                f"CAPTURE PLAN CHECK: {display_status}",
                f"status: {status}",
                f"Exit code: {exit_code}",
                "",
                "Blockers:",
            ...
            lines.extend(
                [
                    "",
                    f"Report: {paths.get('report') or rel(CAPTURE_PLAN_CHECK_REPORT)}",
                    f"Summary: {paths.get('summary') or rel(CAPTURE_PLAN_CHECK_SUMMARY)}",
                    f"Log: {paths.get('log') or rel(CAPTURE_PLAN_CHECK_LOG)}",
                    f"Operator report: {rel(report_path)}",
                ]
            )
            ...
            lines.extend(
                [
                    "",
                    "Safety: metadata-only plan validation; no capture, movement/input, memory scan/read, offset validation, RiftReader validation, or /reloadui was run.",
                ]
            )
            return "\n".join(lines)
    ...
    def run_movement_execution_gate(self) -> None:
        def task() -> str:
            if not MOVEMENT_EXECUTION_GATE_CMD.exists():
                return f"ERROR: missing {rel(MOVEMENT_EXECUTION_GATE_CMD)}"

            exit_code, stdout, stderr = run_command(
                ["cmd", "/c", str(MOVEMENT_EXECUTION_GATE_CMD), "--strict-exit-code"],
                timeout=240,
            )
            summary = load_json(MOVEMENT_EXECUTION_GATE_SUMMARY)
            report_path = write_operator_report()
            ...
            if summary.get("_missing") or summary.get("_error"):
                display_status = "FAIL"
                status = "summary_unavailable"
                blockers = [summary.get("_error") or f"Missing summary: {rel(MOVEMENT_EXECUTION_GATE_SUMMARY)}"]
                paths = {
                    "report": rel(MOVEMENT_EXECUTION_GATE_REPORT),
                    "summary": rel(MOVEMENT_EXECUTION_GATE_SUMMARY),
                    "log": rel(MOVEMENT_EXECUTION_GATE_LOG),
                }
                movement_execution_allowed = False
                expires_utc = None
            else:
                ...
                blockers = list(summary.get("blockers") or [])
                paths = summary.get("paths") or {}
                movement_execution_allowed = summary.get("movement_execution_allowed")
                expires_utc = summary.get("expires_utc")

            if exit_code not in (0, 2) and display_status not in ("PASS", "BLOCKED"):
                display_status = "FAIL"

            lines = [
                "\n=== MOVEMENT EXECUTION GATE ===",
                f"MOVEMENT EXECUTION GATE: {display_status}",
                f"status: {status}",
                f"movement_execution_allowed: {movement_execution_allowed}",
                f"expires_utc: {expires_utc}",
            ...
            lines.extend(
                [
                    "",
                    f"Report: {paths.get('report') or rel(MOVEMENT_EXECUTION_GATE_REPORT)}",
                    f"Summary: {paths.get('summary') or rel(MOVEMENT_EXECUTION_GATE_SUMMARY)}",
                    f"Log: {paths.get('log') or rel(MOVEMENT_EXECUTION_GATE_LOG)}",
                    f"Operator report: {rel(report_path)}",
                ]
            )
            ...
            lines.extend(
                [
                    "",
                    "Safety: final no-input gate only; may run focus and live-wrapper preflight/RiftReader anchor checks, but no capture, movement/input, memory scan/read, offset validation, or /reloadui was run.",
                ]
            )
            return "\n".join(lines)

        self.run_async("movement execution gate", task)
    ...
    def run_capture_readiness_self_test(self) -> None:
        def task() -> str:
            ...
            exit_code, stdout, stderr = run_command(
                ["cmd", "/c", str(CAPTURE_READINESS_CMD), "--self-test"],
                timeout=90,
            )
            summary: dict[str, Any] = {}
            json_start = stdout.find("{")
            ...
            lines = [
                "\n=== CAPTURE READINESS SELF-TEST ===",
                f"CAPTURE READINESS SELF-TEST: {display_status}",
                f"Exit code: {exit_code}",
                f"Cases: {case_count}",
                f"Failing cases: {len(failing)}",
                "",
        ...
        self.run_async("capture readiness self-test", task)
    ...
    def show_guided_workflow_clicked(self) -> None:
        guide = (
            ...
            "7. Movement Execution Gate (no-input final gate; may BLOCK on stale live evidence)\n"
            "8. Run Window/Process Metadata Collector\n"
            "9. Analyze Latest Session\n"
            "10. Compare Sessions\n"
