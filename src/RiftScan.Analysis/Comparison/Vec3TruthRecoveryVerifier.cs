...
public sealed class Vec3TruthRecoveryVerifier
{
    public Vec3TruthRecoveryVerificationResult Verify(string path)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(path);

        var fullPath = Path.GetFullPath(path);
        var issues = new List<Vec3TruthRecoveryVerificationIssue>();
        if (!File.Exists(fullPath))
        {
            issues.Add(Error("file_missing", "Vec3 truth recovery file does not exist."));
            return new Vec3TruthRecoveryVerificationResult { Path = fullPath, Issues = issues };
        }

        Vec3TruthRecoveryResult? result;
        try
        {
            result = JsonSerializer.Deserialize<Vec3TruthRecoveryResult>(File.ReadAllText(fullPath), SessionJson.Options);
        }
        catch (JsonException ex)
        {
            issues.Add(Error("json_invalid", $"Invalid vec3 truth recovery JSON: {ex.Message}"));
            return new Vec3TruthRecoveryVerificationResult { Path = fullPath, Issues = issues };
        }

        if (result is null)
        {
            issues.Add(Error("json_empty", "Vec3 truth recovery JSON did not contain an object."));
            return new Vec3TruthRecoveryVerificationResult { Path = fullPath, Issues = issues };
        }

        ValidateResult(result, issues);
        return new Vec3TruthRecoveryVerificationResult
        {
            Path = fullPath,
    ...
    private static void ValidateResult(Vec3TruthRecoveryResult result, ICollection<Vec3TruthRecoveryVerificationIssue> issues)
    {
        if (!string.Equals(result.SchemaVersion, "riftscan.vec3_truth_recovery.v1", StringComparison.Ordinal))
        {
            issues.Add(Error("schema_version_invalid", "schema_version must be riftscan.vec3_truth_recovery.v1."));
        }

        if (!result.Success)
        {
            issues.Add(Error("result_not_successful", "success must be true for a usable vec3 truth recovery packet."));
        }
        ...
        if (!result.Warnings.Contains("vec3_recovery_is_repeated_candidate_evidence_not_unconditional_coordinate_truth", StringComparer.OrdinalIgnoreCase))
        {
            issues.Add(Error("truth_claim_warning_missing", "warnings must include vec3_recovery_is_repeated_candidate_evidence_not_unconditional_coordinate_truth."));
        }
    ...
    private static void ValidateCandidates(
        IReadOnlyList<Vec3RecoveredTruthCandidate> candidates,
        ICollection<Vec3TruthRecoveryVerificationIssue> issues)
    {
        for (var index = 0; index < candidates.Count; index++)
        {
            ...
            {
                issues.Add(Error("candidate_data_type_invalid", "recovered vec3 candidates must use data_type=vec3_float32.", index));
            }
    ...
    private static Vec3TruthRecoveryVerificationIssue Error(string code, string message, int? candidateIndex = null) =>
        new()
        {
            Code = code,
            Message = message,
